## Quantlet 7 - DiffDiffEstimation D&D Analysis Load Packages used in Q7
library(plm)
...
# treatment
did_4.2 = plm(Delta.Log.Full.Employment ~ binary_treatment1 + year15.dummy + binary1, data = estimation, 
    index = c("State.of.Residence", "Wave"), model = "within")
did_5.2 = plm(Delta.Log.Part.Employment ~ binary_treatment1 + year15.dummy + binary1, data = estimation, 
    index = c("State.of.Residence", "Wave"), model = "within")
did_6.2 = plm(Delta.Log.Marginal.Employment ~ binary_treatment1 + year15.dummy + binary1, data = estimation, 
    index = c("State.of.Residence", "Wave"), model = "within")

# Regression: We regress on Change in Log Employment Status Using Kaitz2013 and robust binary treatment
did_4.3 = plm(Delta.Log.Full.Employment ~ binary_treatment2 + year15.dummy + binary2, data = estimation, 
    index = c("State.of.Residence", "Wave"), model = "within")
did_5.3 = plm(Delta.Log.Part.Employment ~ binary_treatment2 + year15.dummy + binary2, data = estimation, 
    index = c("State.of.Residence", "Wave"), model = "within")
did_6.3 = plm(Delta.Log.Marginal.Employment ~ binary_treatment2 + year15.dummy + binary2, data = estimation, 
    index = c("State.of.Residence", "Wave"), model = "within")

# Output Regression 1: We regress on Log Regular Employment Rate
stargazer(did_1.1.1, did_1.1.2, did_1.2.1, did_1.2.2, title = "Results", type = "text", align = TRUE, no.space = FALSE, 
    keep.stat = c("n", "adj.rsq", "rsq"), dep.var.labels = ("Panel A: Log Regular Employment Rate"), order = c(1, 
        7, 2, 3, 4, 5, 6), covariate.labels = c("Bite.K x D2015", "Bite.F x D2015", "Population(log,t)", 
...
stargazer(did_2.1.1, did_2.1.2, did_2.2.1, did_2.2.2, title = "Results", type = "text", align = TRUE, no.space = FALSE, 
    keep.stat = c("n", "adj.rsq", "rsq"), dep.var.labels = ("Panel B: Log Part Time Employment Rate"), 
...
stargazer(did_3.1.1, did_3.1.2, did_3.2.1, did_3.2.2, title = "Results", type = "text", align = TRUE, no.space = FALSE, 
    keep.stat = c("n", "adj.rsq", "rsq"), dep.var.labels = ("Panel C: Log Marginal Employment Rate"), covariate.labels = c("Bite.K x D2015", 
        "Bite.F x D2015", "Population(log,t)", "Bite.K x D2013", "Bite.K x D2012", "Bite.F x D2013", "Bite.F x D2012"), 
    order = c(1, 7, 2, 3, 4, 5, 6), column.labels = c("Kaitz", "Fraction"), column.separate = c(2, 2))

# Regression 4: We regress on Change in Log Employment Status Using Kaitz2013
stargazer(did_4.1, did_5.1, did_6.1, title = "Results", type = "text", align = TRUE, no.space = TRUE, keep.stat = c("n", 
    "adj.rsq", "rsq"), dep.var.labels.include = FALSE, dep.var.caption = ("Panel D: Log Employment Status"), 
...
# treatment
stargazer(did_4.2, did_5.2, did_6.2, title = "Results", type = "text", align = TRUE, no.space = FALSE, 
...
# treatment
stargazer(did_4.3, did_5.3, did_6.3, title = "Results", type = "text", align = TRUE, no.space = FALSE, 
