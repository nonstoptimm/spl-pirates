# Quantlet 3 Load Packages used in Q3
library(dplyr)
# Execution of Q1 is necessary beforehand

#### Simulation of Minimum Wage #### Function to gather average wages for employees affected by
#### minimum wage for each employment status
minwage_affect = function(x) {
    x %>% group_by(Employment.Status, Subject.to.minwage) %>% summarise(n(), avg_Age = mean(Age, 
        na.rm = TRUE), avg_Sex = mean(Sexnum, na.rm = TRUE), avg_Hourly.earnings = mean(Hourly.earnings), 
...
# Employment effect if Neo Classical Labor market in % Formula: 1 - (wmin / w)^(-x), with wmin:
# minimum wage, m:average gross hourly rate and x: labor demand elasticity Input Data Frame
...
# Employment effect if Monopsonic Labor Market: Two alternative formulas, depend on average
# wage 'w' and market power 'm' If w^min > w(1+0.5m) use: 1 - (w^min / w(1 +m))^(-x) If w^min
# <= w(1+0.5m) use: (w^min - w / 0.5m * w) * ( 1 - (1+0.5 / (1+m))^-x) x =
# Labor.Demand.Elasticity Vector y = input data m = Market Power Create Function
# employ_effect_monopsonic for it
...
# Apply the employ_effect_monopsonic function for the dataset used before with market power of
# 0.2
