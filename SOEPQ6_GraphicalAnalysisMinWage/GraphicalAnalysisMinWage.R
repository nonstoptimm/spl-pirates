# Quantlet 6 Graphical Analysis for Employment Rates of Treatment and Control Groups### Load
# Packages used in Q6
library(ggplot2)
library(sp)
# Execution of Q1 is necessary beforehand!

### Combine Information of Q4 and Q5 We use the averaged data from Q4 for the employment rates
### and add the Fraction and Keitz Index from Q5
...
### Treatment Identification with Keitz Index Use Median of Keitz Index in 2013 for
### identification 1.Standart treatment: If Kaitz higher than Median the State will be treatmant,
### otherwise control 2.Robust treatment: If Kaitz higher than 60% - Percetil will be treatment,
### if under 40%
...
## Function to aggregate data into standard Treatment and Control group using binary_treatment1
## or binary_treatment2, depending on the input
aggregate_treatment_control = function(x, y) {
    ...
    x = x %>% group_by(Wave, y) %>% summarise(Observation = n(), Avg.Log.Full.Employment = mean(Log.Full.Employment, 
        ...
        na.rm = TRUE), Avg.Delta.Log.Full.Employment = mean(Delta.Log.Full.Employment, na.rm = TRUE), 
        Avg.Delta.Log.Part.Employment = mean(Delta.Log.Part.Employment, na.rm = TRUE), Avg.Delta.Log.Marginal.Employment = mean(Delta.Log.Marginal.Employment, 
            na.rm = TRUE), Avg.Full.Employment.Rate = mean(Full.Employment.Rate, na.rm = TRUE), 
        Avg.Part.Employment.Rate = mean(Part.Employment.Rate, na.rm = TRUE), Avg.Marginal.Employment.Rate = mean(Marginal.Employment.Rate, 
            na.rm = TRUE))
    if (y == 1) {
...
### GRAPHS ### Function to plot log employment of binary treatmentgroup for different employment
### status
plot_treatment = function(x, yaxis, treatment, title) {
    ...
    ggplot(data = x, aes(x = Wave, y = yaxis, group = factor(treatment), colour = factor(treatment))) + 
        geom_line() + geom_point() + labs(title = title, y = "Log Employment level", x = "Years") + 
        geom_vline(xintercept = 5, color = "red") + theme_classic() + scale_colour_discrete(name = "Treatment", 
        labels = c("lower Kaitz than median", "higher Kaitz than median")) + coord_cartesian(xlim = c(1.6, 
        7))
}
...
# ggsave('plots/plot_treatment_t1part.png', plot_treatment_t1part) For Marginal Time
# Employment, T1
...
# ggsave('plots/plot_treatment_ta1marginal.png', plot_treatment_ta1marginal) For Full Time
# Employment, T2
...
# ggsave('plots/plot_treatment_ta2full.png', plot_treatment_ta2full) For Part Time Employment,
# T2
...
# ggsave('plots/plot_treatment_ta2part.png', plot_treatment_ta2part) For Marginal Time
# Employment, T2
...
# MAP PLOTS to illustrate Kaitz, Fraction and Treatment for the German states Read Map File for
# Germany
...
correctState = function(x) {
    ...
    x$State.of.Residence = c("Baden-Württemberg", "Bayern", "Berlin", "Brandenburg", "Bremen", 
...
# PLOT Functions Function to plot the Kaitz or Fraction Index. The dataset created with
# createMapdata has to be used for this
plot_result_index = function(input, mode, highcolor) {
    ...
    ggplot(input, aes(x = long, y = lat, group = group)) + geom_polygon(data = input, aes(fill = mode), 
        alpha = 0.8, color = "black") + scale_fill_gradient(low = "white", high = highcolor) + 
        coord_map() + theme(legend.position = "bottom", panel.grid.major = element_blank(), panel.grid.minor = element_blank(), 
...
plot_result_factor = function(x) {
    ...
    ggplot(x) + aes(x = long, y = lat, group = group, fill = Treatment) + geom_polygon(color = "black") + 
        coord_map() + scale_fill_manual(values = c("white", "blue")) + geom_path(color = "black") + 
        theme(legend.position = "bottom", panel.grid.major = element_blank(), panel.grid.minor = element_blank(), 
            panel.background = element_blank(), axis.line = element_blank(), axis.text.x = element_blank(), 
            axis.text.y = element_blank(), axis.ticks = element_blank(), axis.title.x = element_blank(), 
            axis.title.y = element_blank(), plot.title = element_text(hjust = 0.5), axis.ticks.length = unit(0, 
                "mm"), legend.title = element_blank()) + labs(title = "Binary")
}
...
# SAVE the Plots ggsave('plots/plot-kaitz.png', plot_result_kaitz)
# ggsave('plots/plot-fraction.png', plot_result_fraction) ggsave('plots/plot-factor.png',
# plot_result_kaitz)
