# install.packages('gmodels') library(gmodels) rm(list=ls(all=TRUE))
...
# Treatment individuals: Earned less than the minimum wage of 8.50€ before 2015 and 8.50€ starting in 2015 If individual
# belongs to treatment, they have the value of 1 in the treatment variable, otherwise 0

# Control individuals: Earn slightly more than the treatment We create various controll dummies with different wages, as we
# might fear that the sample size is too small for the control group.  1. Control dummy earns between 8.50€ and 9€ 2.
# Control dummy earns between 8.50€ and 9.50€ 3. Control dummy earns between 8.50€ and 10€ If in given wage, dummy value is
# 1, otherwise 0
...
# variable ourselves Use the variables 'Current Gross Labor Income in Euro' and 'Contracted Working Hours' or 'Hours per
# Week Actual' or 'Actual Work Time Per Week' SET THE CURRENT YEAR We start with the first year of our dataset
y = 1
...
# eliminate the values '-2', '-1' Create hourly wage:
y = 1
for (years in c(datalist)) {
...
#### DER CODE FUNKTIONIERT IRGENDWIE NUR MIT DEM 2014er DATENSATZ? GIBT ES HIER UNTERSCHIEDE? ##### Control Dummies Use a loop
#### function to identify the treatment group for the years 2010 - 2015 We use the generated 'hourly earnings' variable
y = 1
for (years in c(datalist)) {
    current_year = datalist[y]
    current_data = get(current_year)
    ...
    # merged2014$`Control_1`[is.na(merged2014$Control_1)] = 0 table(merged2014$Control_1) 2. Control Dummy
    ...
    # merged2014$`Control_2`[is.na(merged2014$Control_2)] = 0 table(merged2014$Control_2) 3. Control Dummy
    ...
    # merged2014$`Control_3`[is.na(merged2014$Control_3)] = 0 table(merged2014$Control_3)
