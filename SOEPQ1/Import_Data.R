...
### IMPORT, MERGE AND CLEAN ALL DATA ### We need two iterators: i is to step through the list of years, beginning with k is always
### one digit higher than i as it reads the second column of the feature selection list (the first column is the label)
...
# Merge all data into one dataframe and add a column with the respective year, called 'Wave' Create a new dataframe
...
# Removes Spaces in Variable Names and substitues with a . - Necessary for the dplyr package, which is handy for later analysis
# of our data
...
rm(list = datalist)
rm(list = c("clean_labels", "clean_sorted", "cleaned", "current_data", "data_merged", "list_import", "ordered_colnames", "soep_selection", 
    "soep_selection_sub", "current_list", "current_year", "i", "k", "labels", "list_dirs", "list_files", "soep_subcrit", "Wave", 
    "years", "z"))
